...
const MyUpvotedPage = async () => {
  const authUser = await auth();
  if (!authUser) {
    redirect("/");
  }

  const upvotedProducts = await getUpvotedProducts();
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10">
            {upvotedProducts.map((product) => (
              <Link href={`/product/${product.slug}`} key={product.id}>
                <div>
                  <div className="rounded-lg hover:scale-105 transition-transform duration-300 ease-in-out justify-center items-center border">
                    <Image
                      src={product.logo}
                      alt="logo"
                      width={1000}
